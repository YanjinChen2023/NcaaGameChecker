...
query1 = """SELECT * FROM NCAA.teams_mapped;"""
...
ncaa_away = []
for i,x in enumerate(mapped['away_team']):
...
for i,x in enumerate(mapped['difference']):
    if mapped['difference'][i] < 0:
        pick[i] = mapped['away_team'][i]
    else:
        pick[i] = mapped['home_team'][i]
...
for i,x in enumerate(GameResult):
    if mapped['away_team'][x[-1]] == x[0][0][0]:
